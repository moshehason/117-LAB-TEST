const c=document.querySelector('#c'),x=c.getContext('2d'),src=document.createElement('canvas'),s=src.getContext('2d');let W,H,D,t=0,mode=0,p={x:.5,y:.5,d:0};
...
function frame(){requestAnimationFrame(frame);t+=.035;x.fillStyle='#fff';x.fillRect(0,0,W,H);let N=54,top=H*.28,hh=H*.40/N,cx=p.x*W,cy=p.y*H;
for(let i=0;i<N;i++){let y=top+i*hh,m=y+hh/2,yn=(m-top)/(H*.40),dy=(m-cy)/(140*D),f=Math.exp(-dy*dy*1.7),dx=0,sc=1;
if(mode===0){dx=Math.sin(t+i*.25)*2*D+(p.d?f*Math.sin((cx-W/2)/W*Math.PI)*60*D:0);sc=1+(p.d?f*.18:0)}
if(mode===1)dx=Math.sin(t*2+i*.32)*14*D+(p.d?f*35*D:0);
if(mode===2)dx=Math.sin(t+i*.18)*4*D, sc=1+Math.max(0,yn-.35)*(.10+.12*Math.sin(t*1.2+i*.11));
if(mode===3){let side=cx<W/2?-1:1;dx=(p.d?side*f*85*D:Math.sin(t*.8+i*.16)*5*D);sc=1+(p.d?f*.28:0)}
